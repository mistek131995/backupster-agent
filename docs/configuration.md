...
- `Storages[]` — хранилища для бэкапов (S3, SFTP, Azure Blob или WebDAV); у каждого уникальное имя и собственный набор настроек.
...
- `StorageName` — ссылается на запись в `Storages[]` по имени. Должно указывать на S3- или Azure Blob-хранилище — SFTP и WebDAV не поддерживаются.
...
**Работает только на S3 и Azure Blob.** При указании SFTP- или WebDAV-хранилища бэкап завершится с ошибкой и статусом `failed` в дашборде.
...
Каждая запись в `Storages[]` имеет поле `Provider` (`S3`, `Sftp`, `AzureBlob` или `WebDav`) и соответствующий вложенный блок с тем же именем. Лишние блоки для выбранного провайдера игнорируются.
...
> **Файловый бэкап (`FilePaths`) не работает на SFTP- и WebDAV-хранилищах** — у этих протоколов нет дешёвого `HEAD` и префикс-листинга для дедупликации кусков. При непустом `FilePaths` для базы, смотрящей на SFTP/WebDAV, дамп загрузится, файлы будут пропущены с warning.
