...
Агент читает секреты при создании соответствующего сервиса или клиента. Горячая ротация plain-значений, `*Secret`-ссылок, содержимого файлов секретов и переменных окружения не поддерживается: после изменения секрета перезапустите агент.
...
Ключ можно хранить в файле или переменной окружения через `KeySecret`:
...
Если plain-поле `Key` пустое и `KeySecret` не задан, агент стартует, но не запускает бэкапы до настройки ключа. Если `KeySecret` задан, внешний источник должен быть доступен уже при старте агента; недоступный файл, отсутствующая env-переменная, пустое значение, не-base64 или ключ не на 32 байта считаются ошибкой конфигурации и валят старт с понятным сообщением.
