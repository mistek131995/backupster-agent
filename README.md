...
MSSQL пишет и читает `.bak` своей собственной файловой системы — не файловой системы агента. Значит путь, который агент передаёт `sqlcmd`'у в команды `BACKUP DATABASE` / `RESTORE DATABASE ... FROM DISK = '...'`, должен быть валиден с точки зрения SQL Server. Если агент и SQL Server живут на разных хостах или в разных ОС, путь с точки зрения SQL Server и путь с точки зрения агента могут не совпадать.

Для этого в `Connections[]` каждого MSSQL-подключения предусмотрены два поля:

- **`SharedBackupPath`** — путь к каталогу `.bak` **с точки зрения SQL Server**. Идёт в `sqlcmd` как есть.
- **`AgentBackupPath`** (опционально) — путь к тому же каталогу **с точки зрения агента**. Нужен только если пути с обеих сторон отличаются (кросс-ОС). Если не задан — агент использует `SharedBackupPath` и для своих операций тоже.

### Примеры конфигурации

| Сценарий | `SharedBackupPath` | `AgentBackupPath` |
|---|---|---|
| Агент и SQL Server на одном хосте (без Docker) | *(не задано)* | *(не задано)* — fallback на `OutputPath` |
