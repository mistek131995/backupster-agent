...
Этот документ — полный перечень HTTP-запросов, которые агент (`DbBackupAgent`) делает к дашборду. Для каждого запроса указаны метод, URL, заголовки, точная схема тела и пример.
