...
Токен задаётся через `AgentSettings__Token`, через `AgentSettings__TokenSecret__Provider=file` + `AgentSettings__TokenSecret__Path=...`, через `AgentSettings__TokenSecret__Provider=env` + `AgentSettings__TokenSecret__Name=...`, через AWS secret provider (`aws-secrets-manager` / `aws-ssm-parameter`), через Azure Key Vault (`azure-key-vault`), через Google Cloud Secret Manager (`google-secret-manager`) или через HashiCorp Vault / HCP Vault Dedicated (`hashicorp-vault`; см. README агента и `docs/configuration.md`). В теле запроса, query-строке и логах агента токен не передаётся никогда.
...
Имена дампов и манифестов в хранилище формируются провайдером, не агент-фасадом. Для всех PG/MySQL/MSSQL/MongoDB дампов схема единая:
...
- `.archive.gz.enc` — MongoDB logical (`mongodump --archive` → gzip → encrypt).
...
Провайдер секретов `file` не добавляет исходящих сетевых запросов: агент читает файл локально перед использованием соответствующего поля. Провайдер `env` также не добавляет сетевых запросов. Провайдеры `aws-secrets-manager` и `aws-ssm-parameter` добавляют исходящие HTTPS-запросы к AWS Secrets Manager (`GetSecretValue`) или AWS Systems Manager Parameter Store (`GetParameter` с `WithDecryption=true` по умолчанию). AWS-значения повторно читаются при каждом разрешении соответствующего `*Secret`-поля, а SDK-клиенты кешируются только по region/service endpoint. Если секрет или SecureString зашифрован customer-managed KMS key, AWS-сервис выполняет расшифровку через AWS KMS от имени вызывающего principal; агент получает только plaintext-ответ от Secrets Manager/SSM. Стандартная цепочка AWS SDK может дополнительно обращаться к AWS credential provider endpoints, например ECS/EKS web identity/container credentials или EC2 Instance Metadata Service, если такие credentials используются в окружении агента. Провайдер `azure-key-vault` добавляет исходящие HTTPS-запросы к заданному в `*Secret.ServiceUrl` хранилищу Azure Key Vault (`GetSecret`, обычно `https://<имя>.vault.azure.net`). Azure-значения повторно читаются при каждом разрешении соответствующего `*Secret`-поля, а SDK-клиенты кешируются только по адресу хранилища. Аутентификация — стандартная цепочка `DefaultAzureCredential` (Azure.Identity), которая дополнительно обращается к точкам выдачи токенов Microsoft Entra ID (`login.microsoftonline.com` либо authority host из `AZURE_AUTHORITY_HOST`), а при managed identity / workload identity — к локальной службе метаданных (IMDS `169.254.169.254` или endpoint из окружения контейнера/кластера). Провайдер `google-secret-manager` добавляет исходящие HTTPS/gRPC-запросы к Google Cloud Secret Manager (`AccessSecretVersion`): global endpoint `secretmanager.googleapis.com` или regional endpoint `secretmanager.{location}.rep.googleapis.com`, если задан `Location` или regional resource name. Google-значения повторно читаются при каждом разрешении соответствующего `*Secret`-поля, а SDK-клиент кешируется в памяти процесса. Аутентификация — стандартная цепочка Application Default Credentials Google SDK; в зависимости от окружения она может дополнительно обращаться к OAuth/STS/IAM endpoints Google или к metadata server/workload identity endpoint окружения. Провайдер `hashicorp-vault` добавляет исходящие HTTP/HTTPS-запросы к `VaultSecretProviders[].Address`: `GET /v1/{mount}/data/{path}?version=N` для KV v2, а при `Auth.Method=AppRole` также `POST /v1/{authMount}/login`. Renewable AppRole token обновляется через `POST /v1/auth/token/renew-self`. В режиме `SecretIdMode=ResponseWrappingToken` агент перед login вызывает `POST /v1/sys/wrapping/lookup`, проверяет `creation_path`, затем вызывает одноразовый `POST /v1/sys/wrapping/unwrap`. Все Vault-запросы с token содержат `X-Vault-Token`; если задан `Namespace`, дополнительно отправляется `X-Vault-Namespace` (для HCP Vault Dedicated обычно `admin`). Plaintext-значения Vault-секретов повторно читаются при каждом разрешении соответствующего `*Secret`-поля. AppRole client token кешируется в памяти и обновляется до окончания lease; если Vault вернул `lease_duration=0`, token кешируется на 1 час. `role_id`, `secret_id`, wrapping token, Vault client token и значения секретов не пишутся на диск и не логируются.

`AgentSettings.TokenSecret` разрешается один раз на operation-scoped lifecycle. Для одного backup/file-set backup тот же снимок токена используется для differential lookup, Open, progress/heartbeat, Finalize и auto-rebase. Task polling сохраняет снимок на пустых long-poll ответах; после получения задачи тот же снимок используется всеми backup/restore/delete вызовами, progress/heartbeat и финальным PATCH. Если любой из этих запросов получил HTTP 401, агент один раз повторно читает `TokenSecret`. При изменившемся значении текущий снимок атомарно обновляется, а исходный HTTP-запрос повторяется один раз. Неизменившееся значение, ошибка повторного чтения или второй HTTP 401 вызывают `DashboardAuthGuard` и штатную остановку агента. Без HTTP 401 дополнительных обращений к внешнему хранилищу токена внутри lifecycle нет. После завершения задачи следующий polling lifecycle заново читает внешний источник. Снимок существует только в памяти и не меняет URL, заголовки или тела запросов к дашборду.
...
- `HashicorpVaultSecretBackend` — **20 с** на один Vault HTTP-вызов.
...
- `*Secret`-ссылки на внешние источники секретов, имена env-переменных, имена/ARN AWS-секретов и параметров, адреса хранилищ и имена секретов Azure Key Vault, Google Secret Manager project id/location/secret resource names, `VaultSecretProviders[]`, Vault token/AppRole credentials и содержимое этих источников
...
| `databaseType`   | enum (string)?  | `postgres`, `mysql`, `mssql`, `mongoDb`, `fileSet`. Для cron-бэкапов БД не передаётся (`null`) — сервер берёт тип из ранее зарегистрированной `AgentDatabase`. Для file-set — `fileSet`, именно по этому полю сервер отличает file-set run и авто-регистрирует `AgentDatabase` с типом `FileSet` при первом открытии |
