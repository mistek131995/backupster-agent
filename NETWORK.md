...
Все расписания — индивидуальные per-(БД, режим). На одну БД может быть до двух записей (`logical` + `physical`), для file-set — одна запись (обычно с `backupMode: "logical"`). Массив `storageNames` означает, что одну запись расписания агент разворачивает в N независимых запусков бэкапа — по одному на каждое хранилище из массива.
