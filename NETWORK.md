...
- `Connections[].ConnectionUri` (для MongoDB может содержать credentials, TLS-параметры и локальные пути сертификатов)
...
| `host`           | string | Хост из `Connections[].Host`. Для MongoDB с `ConnectionUri` — безопасный host, извлечённый из URI без credentials/query |
| `port`           | int    | Порт из `Connections[].Port`. Для MongoDB с `ConnectionUri` — безопасный port, извлечённый из URI; для `mongodb+srv://` может быть display-only значением без передачи URI |

> **Ни `Username`, ни `Password`, ни `ConnectionUri` не попадают в это тело.** Формируется в `ConnectionSyncService.BuildPayload()` — при изменениях проверяйте, что оно по-прежнему берёт только безопасную топологию. Для MongoDB URI query-параметры (`authSource`, `tlsCAFile`, TLS-настройки и т.п.) наружу не отправляются.
