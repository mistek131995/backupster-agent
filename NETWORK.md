...
- `.tar.gz.enc` — PostgreSQL physical (`pg_basebackup --format=tar --gzip` → encrypt). Для DIFF имя несёт суффикс `_diff` (`{database}_{yyyyMMdd_HHmmss}_diff.tar.gz.enc`).
- `_diff.bak.enc` — MSSQL physical differential (`BACKUP DATABASE ... WITH DIFFERENTIAL` → encrypt).
- `.backup_manifest.enc` — PostgreSQL physical sidecar к каждому full/diff: зашифрованный `backup_manifest` от `pg_basebackup`. Хранится рядом с дампом, нужен следующему DIFF (`--incremental=<path>`) и финальному `pg_combinebackup` на restore.
...
Все расписания — индивидуальные per-(БД, режим). На одну БД может быть до трёх записей (`logical` + `physical` + `physicalDifferential`), для file-set — одна запись (обычно с `backupMode: "logical"`). Массив `storageNames` означает, что одну запись расписания агент разворачивает в N независимых запусков бэкапа — по одному на каждое хранилище из массива.
...
| `overrides[].backupMode` | enum (string)              | `"logical"`, `"physical"` или `"physicalDifferential"`. Вместе с `databaseName` образует ключ записи. Перед запуском `physicalDifferential` агент дополнительно дёргает `GET /api/v1/agent/backup-records/last-successful` (секция 9.5) — если живой Successful Physical не нашёлся, тик пропускается с warning |
