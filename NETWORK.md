...
| `replaceExisting`      | bool    | Агент удалит целевую БД перед import (`DROP DATABASE IF EXISTS`). Применяется **только** при `backupMode=logical` и `DatabaseType=Mssql`; дашборд валидирует это и возвращает 400, если флаг выставлен для несовместимого сочетания. Дефолт `false` |
